...
function AmbientFlagToast({ flag, onView, onDismiss }) {
  ...
  // Top-right, not top-center — the center position covered the Deck's
  // own stat cards entirely in practice, real content should never sit
  // underneath a notification. A left accent bar reads calmer than a
    ...
    <div className="fixed top-16 right-4 z-50 w-72" style={{ maxWidth: "calc(100vw - 2rem)", animation: "sd-flag-in 0.25s ease-out" }}>
      <div className="rounded-lg overflow-hidden shadow-2xl" style={{ background: COLORS.surface, borderTop: `1px solid ${COLORS.border}`, borderRight: `1px solid ${COLORS.border}`, borderBottom: `1px solid ${COLORS.border}`, borderLeft: `3px solid ${color}` }}>
        <div className="p-3">
          ...
          <div className="flex gap-2">
            <button onClick={onView} className="flex-1 text-xs py-1.5 rounded-lg font-semibold" style={{ background: color, color: "#0A1120" }}>View</button>
            <button onClick={onDismiss} className="text-xs px-3 py-1.5 rounded-lg" style={{ color: COLORS.muted, border: `1px solid ${COLORS.border}` }}>Dismiss</button>
          </div>
        </div>
      </div>
